---
name: worktrees
description: Manage Git worktrees as OMCC safe isolated coding lanes for complex, risky, or parallel work.
---

# Worktrees Orchestration Protocol

The `worktrees` skill provides an opinionated, safe orchestration protocol for
managing Git worktrees as isolated coding lanes. Its value is giving the Orchestrator a consistent OMCC workflow for parallel agents, risky experiments, integration review, and cleanup.
...
- Ensure the branch name (e.g. `omccs/<slug>` or custom project convention) does not already exist locally or on remote.
...
```gitignore
# BEGIN oh-my-claude-code-slim worktrees
.slim/worktrees/
.slim/worktrees.json
# END oh-my-claude-code-slim worktrees
```
...
2. Formulate a branch name. Default to `omccs/<slug>` unless project/user conventions dictate otherwise.
