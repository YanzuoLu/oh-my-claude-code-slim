...
- improve their oh-my-claude-code-slim setup based on actual usage;
...
- explain restart requirements for Claude Code config, prompt, agent, skill, MCP,
  or plugin changes.
...
2. Project-local guidance and memories, such as `CLAUDE.md`, `.claude/`,
   `$CLAUDE_CONFIG_DIR` (default `~/.claude`), `.slim/`, notes, checkpoints,
   task progress files, and codemaps.
3. Existing skills, commands, agents, prompt overrides, MCP permissions, and
   oh-my-claude-code-slim configuration.
4. Recent Claude Code logs or session artifacts if they are available and safe to
   inspect.
...
- custom agents and their instructions in `agents/<name>.md` bodies;
- custom commands;
- prompt overrides and append files;
- active oh-my-claude-code-slim agent setup, model routing, skills, and MCP permissions;
...
- When config, prompt, agent, skill, MCP, or plugin files change, tell the user:
  "This should apply on the next Claude Code run; restart Claude Code if you need
  it immediately."
