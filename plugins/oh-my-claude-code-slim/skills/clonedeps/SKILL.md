...
```gitignore
# BEGIN oh-my-claude-code-slim clonedeps
.slim/clonedeps/repos/
# END oh-my-claude-code-slim clonedeps
```

Update `.ignore` so Claude Code can read the cloned source while git still ignores
it:
...
### Step 7: Register Dependency Source in CLAUDE.md

After successful cloning, update the repository root `CLAUDE.md` so future
agents know why the dependency source exists and where to look.

If `CLAUDE.md` already has a `## Cloned Dependency Source` section, update that
...
Ask before removing `.slim/clonedeps.json` or the `CLAUDE.md` section because
